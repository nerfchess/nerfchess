...
for (const b of ALL_BUFFS) if (b.implemented && b.tier>=1 && b.tier<=8) live.set(b.id,b.tier);
for (const n of PLAYABLE_NERFS) if (n.tier>=1&&n.tier<=8) live.set(n.id,n.tier);
const regm = new Map<string,number>(reg.entries.map((e:any)=>[e.id,e.tier]));
