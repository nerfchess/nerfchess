...
Spotting generic look-alikes (the owner directive): `--sheet NAME` adds `NAME.sheet.png`, one tile per card of this run at its most card-specific moment, and `NAME.similar.json`, every pair of those cards ranked by how alike their strips look. The sheet tile is chosen among the tiles where the card draws at least half of its own peak energy (raw mean absolute difference from the pre-play tile), as the one whose energy-weighted look is furthest from what the other cards show at that time. The shared wrapper's dim, ring and beam cancel out, and near-empty tiles (just after the reference, the settled tail) can never win; `similar.json` says which tile each card got (`sheetPicks`). Only the cards captured in the run go on an in-run sheet; stale sidecars already in `--out` are ignored. On the godPlays template sample the three top pairs are all GodDescent cards (transcendence, mass_mind_control, full_pardon, checkmate_denial; `sheet/godplays-templates.*`). `similar.json` also has a `generic` ranking: each card's closeness to the mean look of the OTHER cards in the sheet (leave-one-out), highest first, which is the "could belong to another card" list. Pairs and genericness use the same energy-weighted vectors, so a card's quiet tiles count for little. It is triage, not a verdict. Each card in a sheet run also leaves `<id>.strike.png`, `<id>.tiles.png` and `<id>.look.json` (with per-tile `energy`), so batches captured separately can be ranked together with `--sheet-from DIR[,DIR] --sheet NAME --out DIR` (no dev server needed; older sidecars without `energy` get it recomputed from their tiles sprite, and a tile of a different size is letterboxed, not stretched). `--compare before,after --out DIR` writes `<id>.vs.png` with the before strip above the after strip for every card in both folders (no browser needed), which is the pair to link from a REVAMPED row.
...
Reading the JSON under load: this is `next dev` on a shared 4-core box, so dropped frames and long frames are noisy. A long frame whose script source is a `_next/static/chunks/...` file being evaluated for the first time (for example 3.5s on `cardIconComponents_gen` at the end of an `endless_night` window) is a dev compile, not the play; compare before and after runs taken the same way, and look at the scripts listed per long frame before blaming a scene.
...
|---|---|---|---|---|
| F228 | DONE | `evidence/TC0/verify/` (strips, JSON, off checks), `evidence/TC0/speed/` (anim full/fast x fx 0.5/2), `evidence/TC0/sheet/`, `evidence/TC0/sheet-from/` | a402b59 (stage and strips), then the sheet, compare and speed-evidence commit after it, 14273e9 (sidecars, --sheet-from), 091e7d0 (notes), then the review round 1 fix below | `/dev/plays?id=...` plays any live card on the real Board, any cast square and targets, either side and view, data-anim full/fast/off, --fx-dur 0.5 to 2, all from URL params, scriptable via `window.__cardStage`. `scripts/polish/card-strip.ts` captures a small strip plus JSON per card and verifies data-anim off. Grid view: retired cards removed (they were listed), black-at-bottom toggle, a "stage" link per card. |
...
Resume check (2026-09-23, after the sheet sidecars): five cards from five art sources re-run against :3100 with `--off --sheet`: nova (core), world_lock (godPlays), duelist (basicPlays), hw2_death_knell (cursePlays), twin_queens (greatPlays), all `drive: engine`, visualMs 2365 to 2656, off check pass on all five. A second batch (fm_eclipse_crown from fantasyPlays, mass_freeze from core) was captured separately and the two were ranked together with `--sheet-from`: `evidence/TC0/sheet-from/merged.sheet.png` and `merged.similar.json` (7 cards).

Review round 1 (sheet tile choice): the first `--sheet-from` sheet was wrong. It picked each card's tile from look vectors standardised per tile, which blows a near-empty tile up to full-size noise, and noise is always far from the sheet mean, so all seven cards were shown at +190ms or at the settled tail with none of their scene. Fixed as described in HOW TO USE (energy stored in `look.json`, a half-of-peak energy gate, energy-weighted vectors, a leave-one-out mean for the pick and for genericness, in-run sheets filtered to the run's cards, mismatched tile sizes letterboxed). Both batches were re-captured against :3100 and merged again. Every tile in the new `sheet-from/merged.sheet.png` shows the card's own scene, at least as clearly as the old strike sheet `sheet/verify.sheet.png`: fm_eclipse_crown +1299ms (the eclipse disc), hw2_death_knell +191ms (the crown on the doomed d7 pawn, before the wrapper's banner), world_lock +516ms, mass_freeze +204ms (frost on c7 d7 e7), twin_queens +490ms (the queen), nova +565ms (the burst on d2), duelist +796ms (the dueling rings on d2, inside the +790 to +1165ms peak). New numbers in `merged.similar.json`: most alike hw2_death_knell~twin_queens 0.652, then hw2_death_knell~world_lock 0.623; least card-specific hw2_death_knell 0.753, then mass_freeze 0.689 and twin_queens 0.682; nova the most specific at 0.407.
...
F228 is done: the stage, the strip script (strips, off check, speed matrix, module and tier selection, contact sheet with look-alike and genericness ranking, sheets rebuilt across batches with `--sheet-from`, before/after stacking) and this guide. Verified on ten cards across seven art sources (core, godPlays, fantasyPlays, greatPlays, cursePlays, basicPlays, g01). TC0 owns no play module, so it revamped no card; its card-specific contribution is the triage above (TC0-d, TC0-f, the godPlays template sheet) for the family lanes. Not done: a full-library look-alike sweep (174 live T7-8 cards, about 35s each on this box) was left to the family lanes' own before runs so the heavy slots stay free for revamps.
