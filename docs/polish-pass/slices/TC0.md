...
Spotting generic look-alikes (the owner directive): `--sheet NAME` adds `NAME.sheet.png`, one tile per card at its most card-specific moment (the tile furthest from what every card in the sheet shows at that time, so the shared wrapper's dim, ring and beam cancel out and a card whose own art peaks late is shown at that peak), and `NAME.similar.json`, every pair of cards in the run ranked by how alike their strips look. On the godPlays template sample the three top pairs are all GodDescent cards (transcendence, mass_mind_control, full_pardon, checkmate_denial; `sheet/godplays-templates.*`). `similar.json` also has a `generic` ranking: each card's closeness to the mean look of the sheet, highest first, which is the "could belong to another card" list. It is triage, not a verdict. Each card in a sheet run also leaves `<id>.strike.png`, `<id>.tiles.png` and `<id>.look.json`, so batches captured separately can be ranked together with `--sheet-from DIR[,DIR] --sheet NAME --out DIR` (no dev server needed). `--compare before,after --out DIR` writes `<id>.vs.png` with the before strip above the after strip for every card in both folders (no browser needed), which is the pair to link from a REVAMPED row.
...
|---|---|---|---|---|
| F228 | DONE | `evidence/TC0/verify/` (strips, JSON, off checks), `evidence/TC0/speed/` (anim full/fast x fx 0.5/2), `evidence/TC0/sheet/`, `evidence/TC0/sheet-from/` | a402b59 (stage and strips), then the sheet, compare and speed-evidence commit after it | `/dev/plays?id=...` plays any live card on the real Board, any cast square and targets, either side and view, data-anim full/fast/off, --fx-dur 0.5 to 2, all from URL params, scriptable via `window.__cardStage`. `scripts/polish/card-strip.ts` captures a small strip plus JSON per card and verifies data-anim off. Grid view: retired cards removed (they were listed), black-at-bottom toggle, a "stage" link per card. |
...
Resume check (2026-09-23, after the sheet sidecars): five cards from five art sources re-run against :3100 with `--off --sheet`: nova (core), world_lock (godPlays), duelist (basicPlays), hw2_death_knell (cursePlays), twin_queens (greatPlays), all `drive: engine`, visualMs 2365 to 2656, off check pass on all five. A second batch (fm_eclipse_crown from fantasyPlays, mass_freeze from core) was captured separately and the two were ranked together with `--sheet-from`: `evidence/TC0/sheet-from/merged.sheet.png` and `merged.similar.json` (7 cards; nova~duelist 0.62 most alike, hw2_death_knell the least card-specific at 0.81 against the mean).
