...
| F208 | DONE | `evidence/J/f208-before-off.json` (off: 5200ms, busy), `f208-after-off.json` (31ms, never busy), `f208-after-normal.json` (2600ms spacing kept); spec `e2e/polish/signature-queue.spec.ts` | 7f88095 | Spacing = 2600 x `fxDurationScale()` x `tempoScale(detectTempo())` (off, reduced motion and the low-time hold give 0); a 16ms floor keeps one frame per play so plays never coalesce (R9); busy only when spacing > 0. The 7s deferral cap already lives in `useDraftSequence` (ANIMATIONS_SETTLE_CAP_MS). `test:draft-sequence` 20/20. |
