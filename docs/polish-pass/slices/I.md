...
| F194 | DONE (review round 1 fix) | review1-before/chrome-probe.json (2 FAIL), review1-after/chrome-probe.json (16 ok), review1-after/dock-*.png, review1-after/dockuse-*.png | One entrance per row: .dock-arrive on the newest own card, .m-enter otherwise; framer slide and the box-shadow/background dock-pocket-flash dropped from DockRow. Round 1: the entrance is now mount-only (see below) |
...
| F202 | DONE | review1-after/f202-dead-css-grep.txt | Deleted hover-lift, row-in, aura-breathe, cta-shine, Geometry Dash block, starfield family, tailwind rise/seal and their off-rules, the tv-frame backdrop off-rule; no runtime class names reference them (draft-in is still used by BuffCard and kept) |
...
| F189 | PARTIAL | globals.css | globals.css `cubic-bezier(0.16,1,0.3,1)` replaced by `--ease-out`, which is a different curve (0.2, 0.8, 0.2, 1: a softer start and a less abrupt settle), not a rename; read the before/after strips with that in mind. REQUESTS R7 |
...
| Tier A: Dock rows and pocket | DONE | review1-after/dock-*, review1-after/chrome-probe.json | as F194 |
...
Blocking finding (aafdbf6, DockRow): the entrance class was picked from props that change after mount, so a landed row replayed its entrance. Two paths: the previous newest own row losing `flash` swapped `dock-arrive` for `m-enter`, and a use burst ending swapped `dock-used-burst` back to `m-enter`. CSS restarts an animation whose name changes, so the row slid in again from opacity 0. Confirmed, not disputed.

- Root cause and class: a mount-only animation keyed on live props. Sibling grep in owned files: the notices and the tour already key their enter on mount (`data-leaving` is the only later change); no other owned element picks an entrance class from a changing prop.
- Fix (`src/components/dock/DockRow.tsx`): the entrance is chosen once from the mount props (`useState` initializer) and its class comes off for good on the row's own `animationend` for that name, or when a use burst supersedes it. The arrival sheen is tied to the mount entrance too. Known edge: with Animations off no animation runs, so the class stays on (inert under the backstop); if the player turns animations back on mid-game, rows still wearing it play one entrance, the same as a fresh mount. No hydration risk, no timers.
- Regression check (`scripts/polish/i/chrome-probe.ts`, two new checks at Animations normal): (a) sample the previous newest row every frame from just before a second draft until 600ms after and fail on a running `m-enter` or `dock-arrive` or opacity under 1, and also check the dock grew; (b) fire a use burst on a settled row (new harness button `dock-use` in `/dev/motion`, sets `usedActivation`), wait for `dock-used-burst` to drop, sample 600ms the same way. On aafdbf6's DockRow both fail (review1-before/chrome-probe.json: m-enter running at opacity 0 at +1ms and +45ms); after the fix all 16 checks pass (review1-after/chrome-probe.json). The probe also now waits past the dev server's first-paint reload before clicking.
- Strips re-taken: review1-after/dock-{full,fast,off,reduced}.png and dockuse-*.png (new `dockuse` subject in strips.sh; `ANIMS=full` limits the anim list), review1-before/dockuse-full.png. The 100ms strip interval is too coarse to show the 200ms replay reliably; the probe is the evidence.

Nits handled: `scripts/check-anim-props.ts` BASELINE no longer lists `frx-frame` and `frx-frame-release` (slice K fixed them in d316f38), so `test:anim-props` and `scripts/polish/i/anim-props-test.ts` are green again. F202 now has a saved grep. F189 notes the curve change. R1, R2 and R3 below now say who lowers the baselines. TourCoachOverlay's 200vmax box-shadow spread (the dimming scrim around the spotlight hole, no glow, pre-existing) is flagged for the F161 check-shadows allowlist.

## REQUESTS

- R1 (slice A, `src/components/SettingsBootstrap.tsx`, F186): add `import "@/lib/motion";` at the top (side-effect import installs the framer gate). SettingsBootstrap is a client module; `@/lib/motion` has no `"use client"` and touches `document`, so it must only be imported from client modules, never from a server component. Then slice I turns the check-reduced-motion warning into a failure.
- R2 (BuffCard.tsx:177 and NerfCard.tsx:137, integrator or owner): replace `transition-all duration-200` with `transition-[opacity,transform] duration-2` and `motion-safe:group-hover/card:scale-105` with `motion-on:group-hover/card:scale-105`; then remove both files from `TRANSITION_BASELINE` in scripts/check-anim-props.ts and `OS_VARIANT_BASELINE` in scripts/check-reduced-motion.cjs. Both guards fail on a stale (too high) count, so the fix turns them red until the baseline drops: the integrator should land the file fix and slice I's baseline edit in the same integration step (slice I will do the baseline edit on request, or the integrator can apply it directly: delete the two lines).
- R3 (owners of each file, F190): replace `motion-safe:` with `motion-on:` and `motion-reduce:` with `motion-off:`, and CSS `@media (prefers-reduced-motion: reduce)` blocks with `html[data-anim="off"]` selectors, in the files listed in `OS_VARIANT_BASELINE` (scripts/check-reduced-motion.cjs). Then lower that file's count in the baseline. The guard fails on a stale count, so an owner's correct fix turns it red until the baseline drops; owners may lower their own file's count in the same commit (a one-line edit in a slice I file, allowed for this purpose), or the integrator batches the baseline edits.
