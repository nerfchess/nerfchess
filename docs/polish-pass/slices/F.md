...
| F054 | DONE | `text-input-test.txt` (HEAD: fullwidth, accented, Cyrillic, Greek, circled all pass the filter; after: caught; clean words and Cyrillic prose still pass) | 054e415 | NFKD plus mark stripping plus a small confusables map in `normalize()`. Shared by username checks, chat (worker) and the API routes. |
...
| F107 | DONE | code review; `api-fuzz-after.txt` (tournament flows) | 7a2c609 | Result collection and game creation run through `mapLimit(…, 8)` (board order kept for the insert batch); junk ids 404 before the engine runs. Scheduler stays proposal P-tourney-sched. |
| F108 | DONE | `api-fuzz-after.txt` `[F072]`/`[F075]` flows; `query-plans-0045.json` | 159899b, 8ed3f3e | One indexed `EXISTS` read decides; the two UPDATEs run (in one batch) only when something is unread. `since=` stays proposal P-msg-page. |
| F109 | DONE (indexes) | `evidence/F/query-plans-0045.json` (report throttle: SCAN reports before, covering index after; session sweep: SCAN sessions before, index range after; clubs per owner: SCAN before) | 8ed3f3e | Migration `0045_api_request_indexes.sql` with its `schema.ts` mirror (ADDITIVE_COLUMNS): `idx_messages_pair`, `idx_reports_reporter`, `idx_rule_suggestions_user`, `idx_sessions_expires`, `idx_clubs_owner`. Pruning `login_attempts` (now also holding `rl:` keys) needs a scheduled sweep: P-idx. |
...
| F095 | PARTIAL | `query-plans-0045.json` (sweep now an index range) | 8ed3f3e | The index is in; deleting only the presented token is in `auth.ts` (A, REQUEST). |
...
| report, suggest, feedback | DONE | report: reason enum, normalised description, game id pattern, 5/day now index-backed; suggest F057; feedback: guarded, game id pattern. |
...
| Sessions and schema indexes | PARTIAL | 0045 in; auth.ts sweep change is A's. |
...
| A-idx-0045 | `migrations/0045_api_request_indexes.sql` + schema mirror | F095, F108, F109, F061 | DONE (8ed3f3e) |
...
| P-rl-prune | The `rl:` limiter keys live in `login_attempts`, which nothing prunes (F109). Fold them into the P-idx scheduled sweep (`DELETE FROM login_attempts WHERE first_at < now - 1 day`). | F109 | L (cron) |
...
- Slice A (`src/lib/server/auth.ts:197-199`), F095: delete only the presented token instead of a table-wide sweep on the request path:
  ...
  ```
  (`idx_sessions_expires` from 0045 makes the eventual cron sweep an index range.)
...
- Integrator: flag F046 (same-origin and JSON content type on cookie writes) and the private no-store on the tournament detail (it carries seat tokens) as security in the PR. Migration 0045 is additive and index-only.
