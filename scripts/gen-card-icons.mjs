// Build-time generator for the card-id -> lucide icon assignment that used to
// be computed in the browser at module load. It writes TWO files:
//
//   src/lib/cardIconNames.gen.ts       id -> icon NAME map + the registry's
//                                      name set (small strings, loaded eagerly)
//   src/lib/cardIconComponents.gen.ts  the per-icon lucide imports and the
//                                      name -> component registry (loaded
//                                      LAZILY by src/lib/cardIcon.ts, so the
//                                      ~1500 icon components stay out of every
//                                      first-paint chunk)
//
//   npm run gen:icons          # regenerate both files
//   node scripts/gen-card-icons.mjs --check   # CI: fail if either is stale
//
...
const NAMES_FILE = path.join(ROOT, "src", "lib", "cardIconNames.gen.ts");
const COMPONENTS_FILE = path.join(ROOT, "src", "lib", "cardIconComponents.gen.ts");
...
const banner = (what) => [
  ...
  "//",
  `// Inputs: ${cards.length} cards (${ALL_NERFS.length} nerfs + ${ALL_BUFFS.length} buffs, de-duped), lucide catalog ${n}.`,
  what,
];

// File 1: names only. Eager: it is what every card face needs synchronously
// (which glyph, which variant, whether a wire-received icon name is one we
// ship), and it is a few tens of KB of strings.
const namesLines = banner("// Names only: the component registry lives in cardIconComponents.gen.ts.");
namesLines.push("");
namesLines.push("/** Card id -> its globally unique PascalCase lucide face-icon name, for");
namesLines.push(" * every card in the shipped libraries. Sorted by id. */");
namesLines.push("export const CARD_ICON_NAMES: Record<string, string> = {");
for (const id of mapIds) namesLines.push(`  ${id}: ${JSON.stringify(assigned.get(id))},`);
namesLines.push("};");
namesLines.push("");
namesLines.push("/** Every PascalCase icon name the component registry ships (the map above");
namesLines.push(" * plus every card's own valid `icon` field), so a name can be validated");
namesLines.push(" * without loading the components. Sorted. */");
namesLines.push("export const CARD_ICON_REGISTRY_NAMES: ReadonlySet<string> = new Set([");
for (const name of registryNames) namesLines.push(`  ${JSON.stringify(name)},`);
namesLines.push("]);");
namesLines.push("");
const namesOutput = namesLines.join("\n");

// File 2: the components. Only ever reached through a dynamic import (see
// cardIcon.ts), so it is its own chunk and never rides along with a page.
const compLines = banner(
  "// Every icon below is a per-icon named import so Next's optimizePackageImports\n" +
    "// tree-shakes lucide-react down to exactly the icons in this chunk. Import this\n" +
    "// file ONLY via the dynamic import in src/lib/cardIcon.ts.",
);
compLines.push("import {");
compLines.push("  type LucideIcon,");
for (const name of registryNames) compLines.push(`  ${name},`);
compLines.push('} from "lucide-react";');
compLines.push("");
compLines.push("/** PascalCase name -> component, for every icon the map (or any card's own");
compLines.push(" * `icon` field) references. This is the full set of lucide icons the card");
compLines.push(" * UI can render, plus the small category rings in cardIcon.ts. */");
compLines.push("export const GEN_ICON_COMPONENTS: Record<string, LucideIcon> = {");
for (const name of registryNames) compLines.push(`  ${name},`);
compLines.push("};");
compLines.push("");
const compOutput = compLines.join("\n");

const outputs = [
  [NAMES_FILE, namesOutput],
  [COMPONENTS_FILE, compOutput],
];

if (CHECK) {
  const stale = outputs.filter(([file, out]) => (fs.existsSync(file) ? fs.readFileSync(file, "utf8") : "") !== out);
  if (stale.length > 0) {
    console.error(
      `[gen-card-icons] ${stale.map(([f]) => path.relative(ROOT, f)).join(", ")} STALE: the card libraries ` +
  ...
  for (const [file, out] of outputs) fs.writeFileSync(file, out);
  console.log(
    `[gen-card-icons] wrote ${outputs.map(([f]) => path.relative(ROOT, f)).join(" + ")}: ${assigned.size} cards, ` +
      `${registryNames.length} icons in registry${warnings ? `, ${warnings} warning(s)` : ""}.`,
  );
}
