...
function PlayerIdentity({
  ...
  size?: number;
  strong?: boolean;
  /** Sits above a stretched row button: takes pointer events back and
   *  stacks over it. */
  raised?: boolean;
}) {
  return (
    <Link
      ...
      className={
        "group flex min-h-[44px] min-w-0 items-center gap-2 [@media(pointer:fine)]:min-h-0" +
        (raised ? " pointer-events-auto relative" : "")
      }
...
// The channel comes in as a prop from the server page (page.tsx reads
// searchParams), so there is no useSearchParams and no Suspense boundary:
// the header, the heading and the board frame are in the server HTML (F019).
export function TvView({ modeFilter }: { modeFilter: DraftMode | null }) {
  ...
  // Disappearance exits fullscreen. Clamped during render so the overlay
  // never lingers over an absent board.
  const hasBoard = !!(shownId && shownPlayers);
  if (fullscreen && !hasBoard) setFullscreen(false);
  // The fullscreen overlay is a modal dialog (F139): Escape closes it, focus
  // moves in and stays in, the page behind does not scroll, and focus goes
  // back to "Watch fullscreen" on close.
  const { attachDialog: attachFullscreen } = useModalChrome(fullscreen && hasBoard, () => setFullscreen(false));
  ...
  const finalBadge = (
    <span className="inline-flex items-center rounded-none border border-[color:var(--edge-strong)] bg-[color:var(--bg-zebra)] px-2 py-0.5 text-[12px] font-semibold text-parchment-300">
      Final
    </span>
  );
  ...
  const replayBadge = (
    <span className="inline-flex items-center gap-1.5 rounded-none border border-[rgb(var(--energy-ember-rgb)/0.4)] bg-[rgb(var(--energy-ember-rgb)/0.12)] px-2 py-0.5 text-[12px] font-semibold text-[rgb(var(--energy-ember-rgb))]">
    ...
    boardRegion = (
      <div className="relative">
        ...
        <button
          ...
          className="absolute right-2 top-2 grid h-[44px] w-[44px] place-items-center [@media(pointer:fine)]:h-9 [@media(pointer:fine)]:w-9 rounded-none border border-[color:var(--edge-strong)] bg-[color:var(--bg-base)] text-parchment-200 transition-colors hover:border-[color:var(--edge-strong)] hover:text-gold-leaf"
        >
    ...
    <main className="min-h-screen">
            ...
            <div className="plate p-3 sm:p-4">
              ...
              <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                <div className="flex min-w-0 flex-col gap-2">
                  ...
                  {/* The page's only h1 stays in the tree once a game is on
                      the board (F130); it is visually hidden then, because the
                      seat rows carry the players. */}
                  <h1
                    className={
                      shownPlayers
                        ? "sr-only"
                        : "flex items-center gap-2 font-display text-[15px] font-semibold text-parchment-50"
                    }
                  >
                ...
                <div className="flex shrink-0 items-center gap-1.5">
                  <button
                    ...
                    className="grid h-[44px] w-[44px] place-items-center rounded-none border border-[color:var(--edge)] [@media(pointer:fine)]:h-9 [@media(pointer:fine)]:w-9 text-parchment-200 transition-colors hover:border-[color:var(--edge-strong)] hover:text-gold-leaf disabled:opacity-35 disabled:hover:border-[color:var(--edge)] disabled:hover:text-parchment-200"
                  >
                ...
                <ul className="max-h-[34rem] divide-y divide-[color:var(--edge)] overflow-y-auto">
                  {(showAllGames ? liveGames : liveGames.slice(0, TV_LIST_FOLD)).map((g: MPLobbyGame) => {
                    ...
                    return (
                      ...
                      >
                        {/* The row is one button stretched under its content
                            (F133): the player names are links of their own,
                            raised above it, so no link sits inside a button
                            and a click on the rest of the row still picks the
                            game. */}
                        <button
                          type="button"
                          onClick={() => pickGame(g.id)}
                          aria-current={selected ? "true" : undefined}
                          aria-label={`Watch ${g.players.w.name} vs ${g.players.b.name}`}
                          className="absolute inset-0 h-full w-full"
                        />
                        {selected && (
                          <span
                            className="pointer-events-none absolute inset-y-0 left-0 w-0.5 bg-gold-leaf"
                            aria-hidden
                          />
                        )}
                        <div className="pointer-events-none relative">
                          <div className="flex items-center gap-2">
                            <PlayerIdentity
                              ...
                              raised
                            />
                            <span className="shrink-0 text-[12px] text-parchment-400">vs</span>
                            <PlayerIdentity
      ...
      {fullscreen && hasBoard && (
        <div
          ...
          role="dialog"
          aria-modal="true"
          aria-label={`${title}, fullscreen board`}
          className="fixed inset-0 z-50 grid place-items-center overflow-y-auto overscroll-contain bg-ink-900/95 p-4"
        >
          <button
            type="button"
