...
export function serializeGame(game: NerfGame): GameSnapshot {
  ...
  if (game.buffs) {
    ...
    const { mutations, lastHookMutations, lastEventMutations, mutationBy, clockFx, ...persistent } =
      game.buffs;
    void mutations;
...
export function makeBuffApi(game: NerfGame, me: Color): BuffApi {
  ...
  // Record one observable board change for the timeline/notation layer,
  // stamping the currently-running card (bs.mutationBy) unless it is masked.
  // Bumps the same transient counter `mutated()` did, so the onMovePlayed
  // reveal detection is unchanged.
  const record = (m: PublicMutation) => {
    mutated();
    (bs.lastEventMutations ??= []).push(bs.mutationBy ? { ...m, by: bs.mutationBy } : m);
  };
  return {
    board: game.board,
    me,
    ...
    place: (sq, type, color) => {
      bs.historyDiverged = true;
      record({ op: "summon", sq, type, color });
    ...
    relocate: (from, to) => {
      ...
      bs.historyDiverged = true;
      record({ op: "teleport", from, to });
      game.board.pieces[from] = null;
      game.board.pieces[to] = p;
    },
    swap: (a, b) => {
      // Either square may be empty; the exchange still means one swap. A no-op
      // (both empty, or a === b) records nothing so an inert call reveals no
      // card, matching relocate/removePiece's no-op discipline.
      const pa = game.board.pieces[a];
      const pb = game.board.pieces[b];
      if (a === b || (!pa && !pb)) return;
      bs.historyDiverged = true;
      record({ op: "swap", a, b });
      game.board.pieces[a] = pb;
      game.board.pieces[b] = pa;
    },
    setPieceType: (sq, type) => {
      bs.historyDiverged = true;
      const p = game.board.pieces[sq];
      if (p) {
        record({ op: "transform", sq, into: type });
...
export function playMove(game: NerfGame, move: Move): NerfGame {
  ...
  if (bs) {
    ...
    // A committed move opens a fresh event: any rider mutations its hooks make
    // are collected here for the notation layer (the move itself is notated as
    // SAN, not as a PublicMutation).
    bs.lastEventMutations = undefined;
    ...
    for (const color of bs.diff ? [] : (["w", "b"] as Color[])) {
      ...
      for (const { inst, def } of heldBuffs(game, color)) {
        ...
        bs.mutationBy = inst.id;
        def.onMovePlayed(inst, move, api);
        bs.mutationBy = undefined;
...
export function acquireBuff(game: NerfGame, color: Color, id: string, tier: Tier) {
  ...
  const api = makeBuffApi(game, color);
  // Acquiring a card is its own committed event (a pick or an owner grant):
  // reset the collector and attribute the init hook and any instant effect to
  // this card. A grant's identity is masked later at the projection layer, not
  // here — `by` is transient and never sent raw.
  bs.lastEventMutations = undefined;
  bs.mutationBy = id;
  def.init?.(inst, api);
  ps.buffs.push(inst);
...
export function activateBuff(
  ...
  const effectsBefore = bs.effects.length;
  // One activation is one committed event: reset the mutation collector and
  // attribute everything the effect does to this card.
  bs.lastEventMutations = undefined;
  bs.mutationBy = inst.id;
