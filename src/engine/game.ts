...
export function serializeGame(game: NerfGame): GameSnapshot {
  ...
  if (game.buffs) {
    // lastHookMutations / clockFx are transient-only: never persisted,
    // regenerated by the next apply cycle. Dropping them keeps the snapshot
    // equal to what a fresh replay of the same record produces.
    //
    // `mutations` is NOT transient: fxRng mixes it into the seed of every
    // random card effect, so a game restored without it rolls different
    // squares than the live game and a full replay (the parity fuzz found this
    // on 62 of 200 games: the DO's checkpoint path diverged from the clients).
    // It is always written, as a number, so a reader can tell a snapshot that
    // carries it from an older one that dropped it.
    const { mutations, lastHookMutations, clockFx, ...persistent } = game.buffs;
    void lastHookMutations;
    void clockFx;
    buffs = { ...persistent, mutations: mutations ?? 0 };
  }
