...
const LUCKY_CHARMS: Array<OpenerMeta & { win: string; prize: (api: BuffApi) => void }> = [
  ...
  { id: "four_leaf_clover", name: "Four Leaf Clover", flavor: "Three leaves for the lawn, one for the draft table.", icon: "Clover", win: "your next draft offers three cards", prize: (api) => { api.mine.flags.prepThree = true; } },
  ...
  { id: "evil_eye_bead", name: "Evil Eye Bead", flavor: "Blue glass, unblinking, pinned to the royal hem.", icon: "Gem", win: "your queen cannot be captured during your opponent's next turn", prize: (api) => { const q = mySquares(api.board, api.me, "q")[0]; if (q != null) shield1(api, q); } },
  { id: "dowsing_rod", name: "Dowsing Rod", flavor: "It twitches at hanging pieces instead of water.", icon: "Wand", win: "every currently undefended enemy piece flashes until your opponent replies", prize: (api) => flashSquares(api, undefendedPieces(api.board, api.opp)) },
  { id: "horseshoe_nail", name: "Horseshoe Nail", flavor: "For want of a nail, a whole tier was won.", icon: "Magnet", win: "your next draft offer rolls one tier higher", prize: (api) => { api.mine.flags.bankBonus = 1; } },
...
const SLOW_SEASONS: Array<OpenerMeta & { after: number; what: string; pay: (api: BuffApi) => void }> = [
  ...
  { id: "second_harvest", name: "Second Harvest", flavor: "The field you already reaped owes you one more row.", icon: "Tractor", after: 12, what: "gain a draft reroll", pay: (api) => { api.mine.rerollsLeft = (api.mine.rerollsLeft ?? 0) + 1; } },
];
...
> = [
  ...
  { id: "chaperone", name: "Chaperone", flavor: "Approach her majesty and a throat clears meaningfully.", icon: "Eye", what: "The first enemy piece to move into an attack on your queen is marked until you reply", mark: (move, api) => { const q = mySquares(api.board, api.me, "q")[0]; return q != null && attacksSquare(api.board, move.to, q) ? [move.to] : null; } },
  { id: "gate_ledger", name: "Gate Ledger", flavor: "Every siege engine signs out at the gate.", icon: "ScrollText", what: "The first enemy bishop, rook, or queen to leave its back rank is marked until you reply", mark: (move, api) => ((move.piece === "b" || move.piece === "r" || move.piece === "q") && relRank(api.opp, move.from) === 1 ? [move.to] : null) },
  { id: "border_stamp", name: "Border Stamp", flavor: "Passport, please. Purpose of visit: menace.", icon: "Stamp", what: "The first enemy pawn to reach its fifth rank is marked until you reply", mark: (move, api) => (move.piece === "p" && relRank(api.opp, move.to) === 5 ? [move.to] : null) },
  ...
  { id: "harbor_horn", name: "Harbor Horn", flavor: "One long blast means company on the kingside water.", icon: "Megaphone", what: "The first enemy piece to enter your half on the kingside files (e through h) is marked until you reply", mark: (move, api) => (inHalf(api.me, move.to) && FILE(move.to) >= 4 ? [move.to] : null) },
