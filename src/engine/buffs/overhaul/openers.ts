...
function leapfrog(entry: (typeof LEAPFROGS)[number]): Buff {
  ...
  if (entry.id === "garden_hedge" || entry.id === "piggyback" || entry.id === "tower_bridge") {
...
function backstagePass(entry: (typeof BACKSTAGE)[number]): Buff {
  ...
  if (entry.id === "trapdoor_exit") {
    // Renamed to Bishop's Hatch: the two-square back-rank hop already ignores the
    // single square between it and its destination, which may hold a friendly
    // piece. The description now names that passage explicitly.
    return opener(
      entry,
      "Once, while on your back rank, one of your bishops may hop exactly 2 squares sideways along it, passing over the one square between, which may hold a friendly piece. The destination must be empty.",
      augment(gen),
    );
  }
  ...
  if (entry.id === "prompt_corner" || entry.id === "stage_left") {
    // Preserve the hop; taking it consumes your next unused reroll, if any.
    return opener(entry, `${baseDesc} Taking this hop consumes your next unused reroll, if any.`, {
      ...augment(gen),
      onMovePlayed: (inst, move, api) => {
...
function ballroomStep(entry: (typeof BALLROOM)[number]): Buff {
  ...
  if (entry.id === "tango_dip") {
    // Preserve the narrow king-dash identity; after the dash resolves, gain one
    // draft reroll.
    return opener(entry, `${desc} After the dash resolves, gain one draft reroll.`, {
      ...augment(gen),
      onMovePlayed: (inst, move, api) => {
        if (move.via === inst.id && move.color === api.me) {
          api.mine.rerollsLeft = (api.mine.rerollsLeft ?? 0) + 1;
...
function guardian(entry: (typeof GUARDIANS)[number]): Buff {
  ...
  if (entry.id === "tower_warden") {
    // Retier + the shield does not stop pawn captures.
    return opener(
      entry,
      `The first time an enemy piece attacks ${what}, that rook cannot be captured during your opponent's next turn, except by a pawn. One use.`,
      filterGuard({ exceptPawns: true }),
    );
  }

  if (entry.id === "stable_groom") {
    // Retier + delay: the protection now begins only after the opponent's next
    // move (template: delay activation until after the opponent replies).
    return opener(
      entry,
      `One use: the first time an enemy piece attacks ${what}, that knight cannot be captured for one turn, but the guard begins only after your opponent's next move.`,
      filterGuard({ delay: true }),
    );
  }
